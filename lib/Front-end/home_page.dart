...
class CounterPage extends StatefulWidget {
  CounterPage(this.theme);
  final Theme_overall theme;
...
class _CounterPageState extends State<CounterPage> {
  ...
  bool focused = true, share = false, feedback = false, isswitched, init = true;
  ...
  void initState() {
    ...
    SystemChrome.setPreferredOrientations(
        ...
        setState(() {
         small = widget.theme.small;
         medium = widget.theme.medium;
         large = widget.theme.large;
        });
  ...
  Future<void> load_recent() async {
    recent_books = new List();
    final Future<Database> dbFuture = databaseHelper.initializeDatabase();
    dbFuture.then((databse) async {
      Future<List<Book>> predictionList =
          databaseHelper.get_Recent_BookofList();
    ...
    setState(() {
      if(prefs.getBool(scrolling_vertical) == null)
      { prefs.setBool(scrolling_vertical, false);
      isswitched = false;
      }
      else 
         isswitched = prefs.getBool(scrolling_vertical);
    });
    ...
    if (Platform.isAndroid) {
      SimplePermissions.checkPermission(Permission.WriteExternalStorage)
        ...
        if (externalStoragePermissionOkay) {
          ...
          print("Books fro database lkength is : " +
              books_from_DB.length.toString());
          _files.forEach((element) {
            if (element.path.endsWith("pdf")) {
              ...
              if (books_from_DB.length == 0) {
                ...
                databaseHelper
                    .insertBook(new Book.withdata(element.path, '0', '0'));
  ...
  void search(String f) {
    setState(() {
      replacment = CircularProgressIndicator();
    });
    items = new List();
    favorite = new List();
    if (page_index == 2) {
      ...
      books.forEach((element) {
        switch (page_index) {
          case 0:
            if (element.title.toLowerCase().contains(f.toLowerCase()))
              items.add(Book_widget(element));
            break;
          case 1:
            if (element.title.toLowerCase().contains(f.toLowerCase()) &&
                element.favorite == "1") favorite.add(Book_widget(element));
            break;
          default:
            break;
        }
      });
      replacment = Text("no books found");
    }
                ...
                onTap: () {
                  ...
                  Future.delayed(Duration(milliseconds: 55), () {
                    ...
                    Navigator.push(
                        context,
                        MaterialPageRoute(
                            builder: (context) => pdf_reader(book, () {
                                  load_recent();
                                }))).whenComplete(() {
                      print("out of pdf");
                      setState(() {
    ...
    return GestureDetector(
      onTap: () {
        ...
        setState(() {
          page = val == 0
              ? "My Library"
              : val == 1 ? "Favorite" : val == 2 ? "Recent" : "Settings";
        });
      },
    ...
    return !on
        ? GestureDetector(
            onTap: () async {
              ...
              SharedPreferences prefs = await SharedPreferences.getInstance();
              prefs.setString(themekey, s.toString());
            },
            child: Container(
         ...
         child: Column(
           mainAxisAlignment: MainAxisAlignment.center,
             children: <Widget>[
              ...
              Text("Today a reader, tomorrow a leader.",style: TextStyle(
                fontFamily: 'Heading1',fontSize: 22,color: Colors.black54
              ),),           ],
         ),
      ),
          ),
            ...
            children: <Widget>[
              Expanded(
                  ...
                  child: Container(
                      ...
                      child: Row(
                          mainAxisAlignment: page_index != 3
                              ...
                              Switch(
                               ...
                               onChanged: (value) async{
                                 setState(()  { isswitched = value;
                                    ...
                                    SharedPreferences prefs = await SharedPreferences.getInstance();
                                    prefs.setBool(scrolling_vertical,value);
                                  },
                            ...
                            GestureDetector(
                              onTap: () {
                                ...
                                Future.delayed(Duration(milliseconds: 45), () {
                                  ...
                                  const url =
                                      "https://mail.google.com/mail/u/0/?view=cm&fs=1&to=fethinvrfail@gmail.com";
                                  ...
                                  Row(
                                    ...
                                    children: <Widget>[
                                      ...
                                      GestureDetector(
                                        onTap: () async {
                                          const url =
                                              "https://www.linkedin.com/in/-fethi/";
                                          if (await canLaunch(url)) {
                                            await launch(url);
                                          } else {
                                            throw 'Could not launch $url';
                                      ...
                                      GestureDetector(
                                        onTap: () async {
                                          const url =
                                              "https://www.facebook.com/fethi.psy.3/";
                                          if (await canLaunch(url)) {
                                            await launch(url);
                                          } else {
                                            throw 'Could not launch $url';
